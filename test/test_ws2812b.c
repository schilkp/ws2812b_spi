...
void test_buffer_len(void) {
  TEST_ASSERT_EQUAL_INT(
      WS2812B_REQUIRED_BUFFER(1, WS2812B_PACKING_SINGLE, 0, 0), 24);

  TEST_ASSERT_EQUAL_INT(
      WS2812B_REQUIRED_BUFFER(1, WS2812B_PACKING_DOUBLE, 0, 0), 12);

  TEST_ASSERT_EQUAL_INT(
      WS2812B_REQUIRED_BUFFER(2, WS2812B_PACKING_SINGLE, 1, 4), 53);

  TEST_ASSERT_EQUAL_INT(
      WS2812B_REQUIRED_BUFFER(2, WS2812B_PACKING_DOUBLE, 1, 4), 29);

  TEST_ASSERT_EQUAL_INT(
      WS2812B_REQUIRED_BUFFER(0, WS2812B_PACKING_SINGLE, 0, 0), 0);

  TEST_ASSERT_EQUAL_INT(
      WS2812B_REQUIRED_BUFFER(0, WS2812B_PACKING_SINGLE, 1, 4), 5);
}
...
void test_invalid_config_detected(void) {
  ...
  TEST_ASSERT_FALSE_MESSAGE(ws2812b_init(&h_valid),
                            "Reported correct config as incorrect.");
  ...
  TEST_ASSERT_TRUE_MESSAGE(ws2812b_init(&h_test),
                           "Did not detect invalid config.packing!");
  
  // pulse_len_0
  memcpy(&h_test, &h_valid, sizeof(ws2812b_handle_t));
  h_test.config.pulse_len_0 = 0;
  TEST_ASSERT_TRUE_MESSAGE(ws2812b_init(&h_test),
                           "Did not detect invalid config.pulse_len_0!");
  memcpy(&h_test, &h_valid, sizeof(ws2812b_handle_t));
  ...
  TEST_ASSERT_TRUE_MESSAGE(ws2812b_init(&h_test),
                           "Did not detect invalid config.pulse_len_1!");
  memcpy(&h_test, &h_valid, sizeof(ws2812b_handle_t));
  ...
  TEST_ASSERT_TRUE_MESSAGE(ws2812b_init(&h_test),
                           "Did not detect invalid config.first_bit_0!");
  
  // spi_bit_order
  memcpy(&h_test, &h_valid, sizeof(ws2812b_handle_t));
  h_test.config.spi_bit_order = 0xFF;
  TEST_ASSERT_TRUE_MESSAGE(ws2812b_init(&h_test),
                           "Did not detect invalid config.spi_bit_order!");
  ...
  TEST_ASSERT_TRUE_MESSAGE(ws2812b_init(&h_test),
                           "Did not detect that pulses where same length!");
